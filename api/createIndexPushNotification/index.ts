...
async function createIndexPushNotification() {
	const db = client.db(dbName)
	const collection = db.collection<EmailTrainingNotificationItem>(pushCollectionName)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
	try {
		...
		if (req.method === 'POST') {
			...
			const dbResponse = await createIndexPushNotification()
